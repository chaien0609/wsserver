...
function wsEchoText() {
	...
	if ((times.value % 5) != 0) {
		...
		writeToScreen(wsOutput, "Resetting the value " + oldTimes + " to "
				+ times.value);
	}
	writeToScreen(wsOutput, "Sending " + times.value + " messages of \""
			+ size.value + "\" byte(s)");
	payload = "";
...
function onMessage(evt) {
	if (evt.data == payload) {
		...
		if (wsRxBar.value == "100") {
			...
			writeToScreen(wsOutput, "Total execution time: "
					+ (wsEndTime - wsStartTime) + " ms");
...
function onError(evt) {
	writeToScreen(wsOutput, '<span style="color: red;">ERROR:</span> '
			+ evt.data);
}
